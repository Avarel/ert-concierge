...
impl<'a> System<'a> for PhysicalColliderSys {
    ...
    fn run(&mut self, (colliders, entities, mut pos, mut vel, mut shape, mass): Self::SystemData) {
        ...
        for &ColliderPair { elasticity, entity } in &colliders.0 {
            ...
            if let Some((Shape(poly1), Pos(pos1), Vel(vel1), &Mass(mass1))) =
                joiner.get(entity.0, &entities)
            {
                if let Some((Shape(poly2), Pos(pos2), Vel(vel2), &Mass(mass2))) =
                    joiner.get(entity.1, &entities)
                {
                    if let Some(axis) = crate::collision::find_collision(poly1, poly2) {
                        let (reduced_mass, mass_ratio) =
                            match (!mass1.is_normal(), !mass2.is_normal()) {
                                (true, true) => (0.0, 0.0),
                                (false, true) => (mass1, 1.0),
                                (true, false) => (mass2, 0.0),
                                (false, false) => {
                                    ((mass1 * mass2) / (mass1 + mass2), mass1 / (mass1 + mass2))
                                }
                            };
